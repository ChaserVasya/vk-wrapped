...
echo "[INFO] Starting meme function deployment process..."
...
echo "[INFO] Deploying meme function to Yandex Cloud Functions..."

# Читаем переменные из .prod.env файла
ENV_VARS=$(./parse-env.sh .prod.env)

yc serverless function version create \
  --function-name=vk-wrapped-meme \
  --runtime=nodejs18 \
...
if [ $? -eq 0 ]; then
  ...
  echo "[INFO] Function URL: https://functions.yandexcloud.net/$(yc serverless function get --name=vk-wrapped-meme --format=json | jq -r '.id')"
else
